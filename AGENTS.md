...
Standard commands live in `README.md` and `package.json` (`dev`, `dev:no-ui`, `build`, `db:generate`, `db:migrate`, `test:database`, `test:auth`, `test:career-profile`, `test:conversation`, `test:inbound`, `test:discovery`, `test:evals`, `eval*`, `seed`, `typecheck`, `test:scoring`, `capture*`). There is **no linter** and **no Jest/Vitest** — the quality gates are `pnpm typecheck` (`tsc --noEmit`), the `tsx` smoke scripts under `scripts/`, Eve evals (`pnpm test:evals`), and the offline/live model harness (`pnpm eval` / `eval:*`).

### Multi-LLM / fine-tune harness (`eval:*`)

Complementary to GS-009 Eve evals — measures classify/scoring quality when swapping or fine-tuning models:
...
- **Clerk auth is required for web chat and onboarding.** `CLERK_SECRET_KEY` and `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` come from the Vercel Marketplace Clerk resource (`pnpm dlx vercel env pull .env.local --yes`). `proxy.ts` protects non-public routes; Eve uses `clerkMemberAuth()` then `vercelOidc()` then `localDevMemberAuth()` — never `none()`. Internal member rows are created via `agent/lib/identity.ts`; tools read `attributes.memberId` with `requireMemberCaller()`. Details: `docs/auth.md`. Local Eve TUI still works on loopback through the `local-dev` member. If chat returns Eve's `Authorization is required for this route.`, check Clerk token verification — do not default `CLERK_AUTHORIZED_PARTIES` to localhost.
...
Do not extend the shared Markdown profile, browser-local chat history, anonymous production auth, production `/tmp` persistence, or member-facing LinkedIn/X Playwright capture. Those are migration sources, not target seams. Auth is Clerk + identity module — do not reintroduce `none()` on the Eve HTTP channel.
