...
- **LinkedIn personalization**: Career Identity in `user-profile.md` (via chat `ingest_linkedin_profile` or `pnpm capture:profile`) drives role-family matching. `recommend_roles` maps e.g. Sales Engineer @ Vercel AU → Field/Deployment/SE seats at Decagon/Sierra/Cursor/Fireworks; `find_outreach_targets` returns hiring manager / peer-in-seat / adjacent.
