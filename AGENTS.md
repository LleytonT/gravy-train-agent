...
Standard commands live in `README.md` and `package.json` (`dev`, `dev:no-ui`, `build`, `db:generate`, `db:migrate`, `test:database`, `test:auth`, `test:career-profile`, `test:conversation`, `test:inbound`, `seed`, `typecheck`, `test:scoring`, `capture*`). There is **no linter** and **no test runner** — the quality gates are `pnpm typecheck` (`tsc --noEmit`) and the `tsx` smoke scripts under `scripts/`.
