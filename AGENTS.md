...
Standard commands live in `README.md` and `package.json` (`dev`, `dev:no-ui`, `build`, `db:generate`, `db:migrate`, `test:database`, `test:auth`, `test:career-profile`, `test:conversation`, `test:inbound`, `test:discovery`, `test:evals`, `seed`, `typecheck`, `test:scoring`, `test:telegram-link`, `test:telegram-cold-start`, `capture*`). There is **no linter** and **no test runner** — the quality gates are `pnpm typecheck` (`tsc --noEmit`), the `tsx` smoke scripts under `scripts/`, and Eve evals (`pnpm test:evals`).
