...
2. At “Verify with Telegram”, they prove identity. The success screen’s primary CTA is **Open Telegram**.
...
1. `proxy.ts` keeps marketing + get-started public. `/eve/v1/telegram` (and Twilio / capture-sync webhooks) stay public so Clerk cannot 401 the bot. `/app` redirects to `/get-started?verify=1` when neither a member session nor Clerk session is present. Other protected APIs fall through to Clerk `auth.protect()` only when no member session cookie exists.
