...
| Authentication | Telegram user ID | Texting-first identity; web uses Telegram Login to mint a member session. No email IdP. |
...
- resolve a Telegram user ID to a member (creating the member on first private-chat contact),
...
- A verified Telegram user ID creates or resolves the member. Never treat a username as identity. Web reconnect tokens remain short-lived and single-use.
...
1. Telegram contact (texting the bot creates the member),
2. career snapshot or résumé,
3. goals and constraints,
4. inbound job-alert address instructions,
5. optional web workspace via Telegram Login,
